...
# --- Configuration ---
class Config:
    DATA_PATH = os.path.join(os.path.dirname(__file__), "../data/attack_data.csv") # Changed to relative path
    ...
    MAX_LEN = 128
    BATCH_SIZE = 16
    EPOCHS = 10 # <-- THIS IS THE CHANGE (was 5)
    LEARNING_RATE = 2e-5
...
def main():
    if not os.path.exists(config.DATA_PATH):
        # Try a different relative path if the first fails
        config.DATA_PATH = os.path.join(os.path.dirname(__file__), "data/attack_data.csv")
        if not os.path.exists(config.DATA_PATH):
            raise FileNotFoundError(f"Dataset file not found. Make sure 'attack_data.csv' is in a 'data' folder at the root of your project.")
